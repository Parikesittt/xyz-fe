            ...
            <div class="col-md-4">
              ...
              <div class="mb-3">
                ...
                <CInputGroup class="mb-3">
                  ...
                  <CButton id="button-addon2" type="button" color="primary" @click="showItemDialog = true" :disabled="form.customer_name == undefined">
                      <i class="pi pi-search"></i>
                  </CButton>
                </CInputGroup>
                <ItemCsDialog
                  ...
                  :filters="filtersCustItem"
                  :filtersItem="filtersItem"
                  :isItemSelected="isItemSelected"
                  :lazyParams="lazyParamsCustItem"
                  :lazyParamsItem="lazyParamsItem"
                  :loading="loadingCustItem"
                  :loadingItem="loadingItem"
                  :pagination="paginationCustItem"
                  :paginationItem="paginationItem"
                  @update:modalItemCs="showItemDialog = $event"
                  @update:filters="filtersCustItem = $event"
                  @update:filtersItem="filtersItem = $event"
                  @update:lazyParams="(val) => (lazyParamsCustItem = val)"
                  @update:lazyParamsItem="(val) => (lazyParamsItem = val)"
...
const items = ref([]);
const filtersItem = ref({ global: { value: null, matchMode: 'contains' } });
const selectedItem = ref(null);
const loadingItem = ref(false);
const paginationItem = ref({ total: 0, currentPage: 1, perPage: 10 });
const lazyParamsItem = ref({ page: 1, rows: 10 });

// State untuk customer item
const customerItems = ref([]);
const filtersCustItem = ref({ global: { value: null, matchMode: 'contains' } });
const loadingCustItem = ref(false);
const paginationCustItem = ref({ total: 0, currentPage: 1, perPage: 10 });
const lazyParamsCustItem = ref({ page: 1, rows: 10 });

const modalCustomer = ref(false);
const customerList = ref([]);
const filtersCustomer = ref({ global: { value: null, matchMode: 'contains' } });
const loadingCustomer = ref(false);
const paginationCustomer = ref({ total: 0, currentPage: 1, perPage: 10 });
const lazyParamsCustomer = ref({ page: 1, rows: 10 });
...
const fetchCustomer = async () => {
  ...
  try {
    const response = await axios.get(`/api/master/customer`, {
      params: {
        page: lazyParamsCustomer.value.page,
        search: filtersCustomer.value.global.value || '',
        per_page: lazyParamsCustomer.value.rows
      },
      headers: {
    ...
    paginationCustomer.value.total = response.data.data.total;
  } catch (err) {
...
watch(
  () => filtersCustomer.value.global.value,
  () => {
...
watch(
  [modalCustomer, lazyParamsCustomer],
  ([modal]) => {
    if (modal) {
      fetchCustomer();
    }
...
const fetchItems = async () => {
  ...
  try {
    const response = await axios.get('/api/cek_office_cost_items', {
      params: {
        page: lazyParamsItem.value.page,
        search: filtersItem.value.global.value,
        per_page: lazyParamsItem.value.rows
      }
    });

    items.value = response.data.data.data;
    paginationItem.value.total = response.data.data.total;
  } catch (error) {
...
watch(
  () => filtersItem.value.global.value,
  () => {
...
const fetchCustomerItems = async () => {
  loadingCustItem.value = true;
  try {
    const response = await axios.get(`/api/office_customer_items/${form.value.customer_id}`, {
      params: {
        page: lazyParamsCustItem.value.page,
        search: filtersCustItem.value.global.value || '',
        per_page: lazyParamsCustItem.value.rows
      }
    });

    customerItems.value = response.data.data.data;
    paginationCustItem.value.total = response.data.data.total;
  } catch (error) {
    console.error('Error fetching customer items:', error);
  } finally {
    loadingCustItem.value = false;
  }
};

const debouncedFetchCustomerItems = debounce(() => {
  fetchCustomerItems();
}, 400);

watch(
  () => filtersCustItem.value.global.value,
  () => {
    if (showItemDialog.value) {
      debouncedFetchCustomerItems();
    }
  }
);

watch(
  [showItemDialog, lazyParamsCustItem],
  ([modal]) => {
    if (modal) {
      fetchCustomerItems();
    }
