<template>
  <div>
    ...
    <div class="d-grid gap-2 d-md-flex justify-content-md-end mb-4">
      <CButton color="primary" @click="createCity">
    ...
    >
      ...
      <Column header="Actions">
        <template #body="{data}">
          <Button icon="pi pi-pencil" class="p-button-warning mr-2" @click="editCity(data.id)" />
        </template>
      </Column>
    </DataTable>
  </div>
...
const cityList = ref([]); // List of companies fetched from API
// const list = ref([]);
...
const fetchData = async (pageNumber = 1, keywords = '') => {
  ...
  loading.value = true;
  try {
    const response = await axios.get(`/api/master/city?search=${keywords}&page=${page}`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });

    cityList.value = response.data.data.data; // Set company data
    pagination.currentPage = response.data.data.current_page;
    pagination.perPage = response.data.data.per_page;
    pagination.total = response.data.data.total;

    console.log('City List:', cityList); // Log to verify the data
  } catch (err) {
...
const createCity = () => {
  ...
  router.push({ name: 'Create City' });
};

const editCity = (cityId) => {
  console.log('Redirecting to edit company page with ID:', cityId);
  router.push({ name: 'Edit City', params: { id: cityId } });
};
