<template>
  <div>
    ...
    <div class="d-grid gap-2 d-md-flex justify-content-md-end mb-4">
      <CButton color="primary" @click="createVendor">
    ...
    >
      ...
      <Column header="Actions">
        <template #body="{data}">
          <Button icon="pi pi-pencil" class="p-button-warning mr-2" @click="editVendor(data.id)" />
        </template>
      </Column>
    </DataTable>
  </div>
...
const columnDefs = [
  ...
  { field: 'currency', headerName: 'Currency', sortable: true, flex:1 },
  { field: 'terms', headerName: 'Term', sortable: true, flex:1 },
  {
    field: 'is_active',
    headerName: 'Active',
    sortable: true,
...
const fetchData = async (pageNumber = 1, keywords = '') => {
  ...
  loading.value = true;
  try {
    const response = await axios.get(`/api/master/vendor?search=${keywords}&page=${page}`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });

    vendorList.value = response.data.data.data; // Set company data
    pagination.currentPage = response.data.data.current_page;
    pagination.perPage = response.data.data.per_page;
    pagination.total = response.data.data.total;

    console.log('Vendor List:', vendorList); // Log to verify the data
  } catch (err) {
...
const editVendor = (vendorId) => {
  console.log('Redirecting to edit vendor page with ID:', vendorId);
  router.push({ name: 'Edit Vendor', params: { id: vendorId } });
};
