<template>
  <div>
    <div class="mb-3"><h3>Business Type</h3></div>
    ...
    <div class="d-grid gap-2 d-md-flex justify-content-md-end mb-4">
      <CButton color="primary" @click="createBusType">
    ...
    >
      ...
      <Column header="Actions">
        <template #body="{data}">
          <Button icon="pi pi-pencil" class="p-button-warning mr-2" @click="editBusType(data.id)" />
        </template>
      </Column>
    </DataTable>
  </div>
...
const busTypeList = ref([]); // List of companies fetched from API
// const list = ref([]);
...
const columnDefs = [
  ...
  { field: 'name', headerName: 'Name', sortable: true, flex: 3 },
  {
    field: 'is_active',
    headerName: 'Active',
    sortable: true,
...
const fetchData = async (pageNumber = 1, keywords = '') => {
  ...
  loading.value = true;
  try {
    const response = await axios.get(`/api/master/business_type?search=${keywords}&page=${page}`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });

    busTypeList.value = response.data.data.data; // Set company data
    pagination.currentPage = response.data.data.current_page;
    pagination.perPage = response.data.data.per_page;
    pagination.total = response.data.data.total;

    console.log('Item Subgroup List:', busTypeList.value.created_at); // Log to verify the data
  } catch (err) {
...
const createBusType = () => {
  ...
  router.push({ name: 'Create Business Type' });
};

const editBusType = (busTypeId) => {
  console.log('Redirecting to edit business type page with ID:', busTypeId);
  router.push({ name: 'Edit Business Type', params: { id: busTypeId } });
};
