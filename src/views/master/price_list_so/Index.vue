<template>
  <div>
          ...
          <div class="d-grid gap-2 d-md-flex justify-content-md-end mb-4">
            <CButton color="primary" @click="createSOPriceList">
          ...
          <Column field="is_active" header="Status" :sortable="true">
            ...
            <Column header="Actions">
              <template #body="{data}">
                <Button icon="pi pi-pencil" class="p-button-warning mr-2" @click="editSOPriceList(data.id)" />
              </template>
            </Column>
          </DataTable>
        </div>
...
const soPriceList = ref([]); // List of companies fetched from API
// const list = ref([]);
...
const fetchData = async (pageNumber = 1, keywords = '') => {
  ...
  loading.value = true;
  try {
    const response = await axios.get(`/api/master/price_list_so?search=${keywords}&page=${page}`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });

    soPriceList.value = response.data.data.data; // Set company data
    pagination.currentPage = response.data.data.current_page;
    pagination.perPage = response.data.data.per_page;
    pagination.total = response.data.data.total;

    console.log('Price List:', soPriceList); // Log to verify the data
  } catch (err) {
...
const createSOPriceList = () => {
  console.log('Redirecting to create page');
  router.push({ name: 'Create Price List SO' });
};

const editSOPriceList = (soPriceListId) => {
  console.log('Redirecting to edit customer page with ID:', soPriceListId);
  router.push({ name: 'Edit Price List SO', params: { id: soPriceListId } });
};
