<template>
  <div>
    ...
    <div class="d-grid gap-2 d-md-flex justify-content-md-end mb-4">
      <CButton color="primary" @click="createWarehouse">
    ...
    >
      ...
      <Column field="location.name" header="Location Name" :sortable="true" />
      <Column field="customer" header="Customer" :sortable="true" />

      <Column header="Actions">
        <template #body="{data}">
          <Button icon="pi pi-pencil" class="p-button-warning mr-2" @click="editWarehouse(data.id)" />
        </template>
      </Column>
    </DataTable>
  </div>
...
const columnDefs = [
  ...
  { field: 'name', headerName: 'Warehouse Name', sortable: true, flex: 1 },
  { field: 'location.name', headerName: 'Location', sortable: true, flex:1 },
  { field: 'customer_id', headerName: 'Customer', sortable: true, flex:1 },
  {
    field: 'is_active',
    headerName: 'Active',
    sortable: true,
...
const fetchData = async (pageNumber = 1, keywords = '') => {
  ...
  loading.value = true;
  try {
    const response = await axios.get(`/api/master/warehouse?search=${keywords}&page=${page}`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });

    warehouseList.value = response.data.data.data; // Set company data
    pagination.currentPage = response.data.data.current_page;
    pagination.perPage = response.data.data.per_page;
    pagination.total = response.data.data.total;

    console.log('Branch List:', warehouseList); // Log to verify the data
  } catch (err) {
...
const createWarehouse = () => {
  ...
  router.push({ name: 'Create Warehouse' });
};

const editWarehouse = (warehouseId) => {
  console.log('Redirecting to edit warehouse page with ID:', warehouseId);
  router.push({ name: 'Edit Warehouse', params: { id: warehouseId } });
};
