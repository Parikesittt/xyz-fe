<template>
  <CCard class="mb-4">
    <CCardHeader>
      <strong>{{ isEdit ? 'Edit Office Consumable' : 'Create Office Consumable' }}</strong>
    </CCardHeader>
    <CCardBody>
...
const fetchData = async (id) => {
  try {
    const response = await axios.get(`/api/master/office_consumable/${id}`)
    const reset = response.data.data
    form.value = {
      ...reset,
      is_active: reset.is_active === 1 ? '1' : '0'
    }
    if (reset.office_consumable_items && Array.isArray(reset.office_consumable_items)) {
      // Pemetaan data dari office_consumable_items
      const mappedItemParts = reset.office_consumable_items.map(cons => ({
        idConsumable: cons.id,
...
const save = async () => {
  ...
  formData.append('office_consumable_items', JSON.stringify(selectedItem.value))
  ...
  try {
    ...
    if (isEdit.value) {
      ...
      response = await axios.post(`/api/master/office_consumable/update/${form.value.id}`, formData, {
        headers: { 'Content-Type': 'multipart/form-data' }
      })
      Swal.fire({
                position: "top-end",
                title: 'Berhasil',
                text: 'Office Consumable berhasil diperbarui',
                icon: 'success',
      ...
      // Create new company
      response = await axios.post('/api/master/office_consumable', formData, {
        headers: { 'Content-Type': 'multipart/form-data' }
      })
      Swal.fire({
                position: "top-end",
                title: 'Berhasil',
                text: 'Office Consumable berhasil dibuat',
                icon: 'success',
                showConfirmButton: false,
                timer: 1000
      });
    ...
    router.push('/master/office_consumable') // Redirect to company list page
  } catch (error) {
...
const submit = async () => {
  ...
  try {
    ...
    router.push('/master/office_consumable') // Redirect to cost list page
  } catch (error) {
    console.error('Error saving cost:', error)
  }
}
...
const back = () => {
  router.push({ name: 'Office Consumable' });
};
