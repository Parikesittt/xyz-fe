<template>
  <div>
          ...
          <div class="d-grid gap-2 d-md-flex justify-content-md-end mb-4">
            <CButton color="primary" @click="createConsumable">
          ...
          <Column field="is_active" header="Status" :sortable="true">
            ...
            <Column header="Actions">
              <template #body="{data}">
                <Button icon="pi pi-pencil" class="p-button-warning mr-2" @click="editConsumable(data.id)" />
              </template>
            </Column>
          </DataTable>
        </div>
...
const consumableList = ref([]); // List of companies fetched from API
// const list = ref([]);
...
const fetchData = async (pageNumber = 1, keywords = '') => {
  ...
  loading.value = true;
  try {
    const response = await axios.get(`/api/master/office_consumable?search=${keywords}&page=${page}`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });

    consumableList.value = response.data.data.data; // Set company data
    pagination.currentPage = response.data.data.current_page;
    pagination.perPage = response.data.data.per_page;
    pagination.total = response.data.data.total;

    console.log('Price List:', consumableList); // Log to verify the data
  } catch (err) {
...
const createConsumable = () => {
  ...
  router.push({ name: 'Create Office Consumable' });
};

const editConsumable = (consumableId) => {
  console.log('Redirecting to edit customer page with ID:', consumableId);
  router.push({ name: 'Edit Office Consumable', params: { id: consumableId } });
};
