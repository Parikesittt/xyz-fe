<template>
  <CCard class="mb-4">
    <CCardHeader>
      <strong>{{ isEdit ? 'Edit Office Reset' : 'Create Office Reset' }}</strong>
    </CCardHeader>
    <CCardBody>
...
const fetchData = async (id) => {
  try {
    const response = await axios.get(`/api/master/office_reset/${id}`)
    const reset = response.data.data
    form.value = {
      ...reset,
      is_active: reset.is_active === 1 ? '1' : '0'
    }
    if (reset.office_reset_items && Array.isArray(reset.office_reset_items)) {
      // Pemetaan data dari office_reset_items
      const mappedItemParts = reset.office_reset_items.map(reset => ({
        idReset: reset.id,
        reset_id: reset.reset_id,
        product_code: reset.product_code,
        code: reset.item_code,
        name: reset.item_name,
        unit: reset.item_unit,
        price: reset.price
      }));
...
const save = async () => {
  ...
  formData.append('office_reset_item', JSON.stringify(selectedItem.value))
  formData.append('is_active', form.value.is_active)
  try {
    let response
    if (isEdit.value) {
      // Edit existing company
      response = await axios.post(`/api/master/office_reset/update/${form.value.id}`, formData, {
        headers: { 'Content-Type': 'multipart/form-data' }
      })
      Swal.fire({
                position: "top-end",
                title: 'Berhasil',
                text: 'Office Reset berhasil diperbarui',
                icon: 'success',
      ...
      // Create new company
      response = await axios.post('/api/master/office_reset', formData, {
        headers: { 'Content-Type': 'multipart/form-data' }
      })
      Swal.fire({
                position: "top-end",
                title: 'Berhasil',
                text: 'Office Reset berhasil dibuat',
                icon: 'success',
                showConfirmButton: false,
                timer: 1000
      });
    ...
    router.push('/master/office_reset') // Redirect to company list page
  } catch (error) {
...
const submit = async () => {
  ...
  try {
    ...
    router.push('/master/office_reset') // Redirect to cost list page
  } catch (error) {
    console.error('Error saving cost:', error)
  }
}
