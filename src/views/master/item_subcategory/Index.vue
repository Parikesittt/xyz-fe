<template>
  <div>
    <div class="mb-3"><h3>Item Category</h3></div>
    ...
    <div class="d-grid gap-2 d-md-flex justify-content-md-end mb-4">
      <CButton color="primary" @click="createItemSub">
    ...
    >
      ...
      <Column header="Actions">
        <template #body="{data}">
          <Button icon="pi pi-pencil" class="p-button-warning mr-2" @click="editItemSub(data.id)" />
        </template>
      </Column>
    </DataTable>
  </div>
...
const fetchData = async (pageNumber = 1, keywords = '') => {
  ...
  loading.value = true;
  try {
    const response = await axios.get(`/api/master/item_sub?search=${keywords}&page=${page}`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });

    itemSubList.value = response.data.data.data; // Set company data
    pagination.currentPage = response.data.data.current_page;
    pagination.perPage = response.data.data.per_page;
    pagination.total = response.data.data.total;

    console.log('Item Category List:', itemSubList.value.created_at); // Log to verify the data
  } catch (err) {
...
const createItemSub = () => {
  ...
  router.push({ name: 'Create Item Category' });
};

const editItemSub = (itemSubId) => {
  console.log('Redirecting to edit company page with ID:', itemSubId);
  router.push({ name: 'Edit Item Category', params: { id: itemSubId } });
};
