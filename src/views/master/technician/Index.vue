<template>
  <div>
    <div class="mb-3"><h3>Technician</h3></div>
    ...
    <div class="d-grid gap-2 d-md-flex justify-content-md-end mb-4">
      <CButton color="primary" @click="createTechnician">
    ...
    >
      ...
      <Column header="Actions">
        <template #body="{data}">
          <Button icon="pi pi-pencil" class="p-button-warning mr-2" @click="editTechnician(data.id)" />
        </template>
      </Column>
    </DataTable>
  </div>
...
const technicianList = ref([]); // List of companies fetched from API
// const list = ref([]);
...
const columnDefs = [
  ...
  { field: 'phone', headerName: 'Phone', sortable: true, flex: 2 },
  {
    field: 'is_active',
    headerName: 'Active',
    sortable: true,
...
const fetchData = async (pageNumber = 1, keywords = '') => {
  ...
  loading.value = true;
  try {
    const response = await axios.get(`/api/master/technician?search=${keywords}&page=${page}`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });

    technicianList.value = response.data.data.data; // Set company data
    pagination.currentPage = response.data.data.current_page;
    pagination.perPage = response.data.data.per_page;
    pagination.total = response.data.data.total;

    console.log('Item Subgroup List:', technicianList.value.created_at); // Log to verify the data
  } catch (err) {
...
const createTechnician = () => {
  ...
  router.push({ name: 'Create Technician' });
};

const editTechnician = (technicianId) => {
  console.log('Redirecting to edit technician page with ID:', technicianId);
  router.push({ name: 'Edit Technician', params: { id: technicianId } });
};
