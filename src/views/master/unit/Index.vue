<template>
  <div>
    ...
    <div class="d-grid gap-2 d-md-flex justify-content-md-end mb-4">
      <CButton color="primary" @click="createUnit">
    ...
    >
      ...
      <Column header="Actions">
        <template #body="{data}">
          <Button icon="pi pi-pencil" class="p-button-warning mr-2" @click="editUnit(data.id)" />
        </template>
      </Column>
    </DataTable>
  </div>
...
const unitList = ref([]); // List of companies fetched from API
// const list = ref([]);
...
const columnDefs = [
  ...
  { field: 'desc', headerName: 'Description', sortable: true, flex: 2 },
  {
    field: 'created_at',
    headerName: 'Date',
    sortable: true,
...
const fetchData = async (pageNumber = 1, keywords = '') => {
  ...
  loading.value = true;
  try {
    const response = await axios.get(`/api/master/unit?search=${keywords}&page=${page}`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });

    unitList.value = response.data.data.data; // Set company data
    pagination.currentPage = response.data.data.current_page;
    pagination.perPage = response.data.data.per_page;
    pagination.total = response.data.data.total;

    console.log('Item Subgroup List:', unitList.value.created_at); // Log to verify the data
  } catch (err) {
...
const createUnit = () => {
  ...
  router.push({ name: 'Create Unit' });
};

const editUnit = (unitId) => {
  console.log('Redirecting to edit unit page with ID:', unitId);
  router.push({ name: 'Edit Unit', params: { id: unitId } });
};
