<template>
  <div>
    <div class="mb-3"><h3>Item Super Group</h3></div>
    ...
    <div class="d-grid gap-2 d-md-flex justify-content-md-end mb-4">
      <CButton color="primary" @click="createItemSuperGroup">
    ...
    >
      ...
      <Column header="Actions">
        <template #body="{data}">
          <Button icon="pi pi-pencil" class="p-button-warning mr-2" @click="editItemSuperGroup(data.id)" />
        </template>
      </Column>
    </DataTable>
  </div>
  ...
  },
  {
    ...
    cellRenderer: (params) => {
      ...
      editButton.addEventListener('click', () => {
        ...
        editItemSuperGroup(params.data.id);
      });
      return editButton;
    },
    flex:1
...
const fetchData = async (pageNumber = 1, keywords = '') => {
  ...
  loading.value = true;
  try {
    const response = await axios.get(`/api/master/item_super_group?search=${keywords}&page=${page}`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });

    itemSupList.value = response.data.data.data; // Set company data
    pagination.currentPage = response.data.data.current_page;
    pagination.perPage = response.data.data.per_page;
    pagination.total = response.data.data.total;

    console.log('Item Super Group List:', itemSupList.value.created_at); // Log to verify the data
  } catch (err) {
...
const createItemSuperGroup = () => {
  console.log('Redirecting to create page');
  router.push({ name: 'Create Item Super Group' });
};

const editItemSuperGroup = (itemSupId) => {
  console.log('Redirecting to edit company page with ID:', itemSupId);
  router.push({ name: 'Edit Item Super Group', params: { id: itemSupId } });
};
