<template>
  <div>
    <div class="mb-3"><h3>Financial Statement</h3></div>
    ...
    <div class="d-grid gap-2 d-md-flex justify-content-md-end mb-4">
      <CButton color="primary" @click="createFS">
    ...
    >
      ...
      <Column header="Actions">
        <template #body="{data}">
          <Button icon="pi pi-pencil" class="p-button-warning mr-2" @click="editFS(data.id)" />
        </template>
      </Column>
    </DataTable>
  </div>
...
const fsList = ref([]); // List of companies fetched from API
// const list = ref([]);
...
const fetchData = async (pageNumber = 1, keywords = '') => {
  ...
  loading.value = true;
  try {
    const response = await axios.get(`/api/master/financial_statement?search=${keywords}&page=${page}`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });

    fsList.value = response.data.data.data; // Set company data
    pagination.currentPage = response.data.data.current_page;
    pagination.perPage = response.data.data.per_page;
    pagination.total = response.data.data.total;

    console.log('Item Subgroup List:', fsList.value.created_at); // Log to verify the data
  } catch (err) {
...
const createFS = () => {
  ...
  router.push({ name: 'Create Financial Statement' });
};

const editFS = (fsId) => {
  console.log('Redirecting to edit Facility page with ID:', fsId);
  router.push({ name: 'Edit Financial Statement', params: { id: fsId } });
};
