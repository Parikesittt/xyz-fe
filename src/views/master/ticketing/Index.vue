<template>
  <div>
    <div class="mb-3"><h3>Ticketing</h3></div>
    ...
    <div class="d-grid gap-2 d-md-flex justify-content-md-end mb-4">
      <CButton color="primary" @click="createTicketing">
    ...
    >
      ...
      <Column header="Actions">
        <template #body="{data}">
          <Button icon="pi pi-pencil" class="p-button-warning mr-2" @click="editTicketing(data.id)" />
        </template>
      </Column>
    </DataTable>
  </div>
...
const ticketingList = ref([]); // List of companies fetched from API
// const list = ref([]);
...
const columnDefs = [
  { field: 'name', headerName: 'Name', sortable: true, flex: 4 },
  {
    field: 'is_active',
    headerName: 'Active',
    sortable: true,
...
const fetchData = async (pageNumber = 1, keywords = '') => {
  ...
  loading.value = true;
  try {
    const response = await axios.get(`/api/master/ticketing?search=${keywords}&page=${page}`, {
      headers: {
        Authorization: `Bearer ${token}`,
      },
    });

    ticketingList.value = response.data.data.data; // Set company data
    pagination.currentPage = response.data.data.current_page;
    pagination.perPage = response.data.data.per_page;
    pagination.total = response.data.data.total;

    console.log('Ticketing List:', ticketingList.value.created_at); // Log to verify the data
  } catch (err) {
...
const createTicketing = () => {
  console.log('Redirecting to create page');
  router.push({ name: 'Create Ticketing' });
};

const editTicketing = (ticketingId) => {
  console.log('Redirecting to edit Ticketing page with ID:', ticketingId);
  router.push({ name: 'Edit Ticketing', params: { id: ticketingId } });
};
